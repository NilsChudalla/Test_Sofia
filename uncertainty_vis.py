...
WE_profile = st.sidebar.slider(label='Profile slider (N-S)', min_value=float(np.min(x_array)), max_value=float(np.max(x_array)),
                       value=float(np.mean(x_array)))
WE_index = np.argmin(np.abs(y_array - WE_profile))
NS_profile = st.sidebar.slider(label='Profile slider (W-E)', min_value=float(np.min(y_array)), max_value=float(np.max(y_array)),
